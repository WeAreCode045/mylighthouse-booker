		...
		function initPicker() {
			...
			try {
				...
				if (contentWrapper) {
					...
					let displayHotelName = 'Hotel'; // fallback
					
					// Try to get selected hotel name from dropdown if present
					...
					// If no selected hotel from dropdown, use data attribute
					if (displayHotelName === 'Hotel') {
						const dataHotelName = $form.data('hotel-name');
						if (dataHotelName && dataHotelName.trim()) {
							displayHotelName = dataHotelName.trim();
						}
					}
					...
					// Determine form type and configure accordingly
					...
					if (isHotelForm) {
						...
						if (roomRow) {
							...
							const roomNameSpan = roomRow.querySelector('.mlb-room-name');
						...
						const ctaRoom = modalOverlay.querySelector('.mlb-modal-cta-room');
						const ctaSpecial = modalOverlay.querySelector('.mlb-modal-cta-special');
						if (ctaRoom) ctaRoom.textContent = 'Book This Room';
						if (ctaSpecial) ctaSpecial.style.display = 'none';
						...
						if (specialRow) {
							...
							const specialNameSpan = specialRow.querySelector('.mlb-special-name');
						...
						if (ctaSpecial) {
							ctaSpecial.style.display = 'block';
							...
							modalSubmitBtn.addEventListener('click', function(e) {
								...
								try {
									...
									if (isHotelForm) {
										...
										// For room/special forms, trigger the normal submission
									...
									console.error('MLB: Error in modal submit handler', e);
