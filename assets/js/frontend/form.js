...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleFormSubmission(bookingForm) {
        ...
        if (target === 'redirect_engine') {
            ...
            const rateId = bookingForm.dataset.rateId || bookingForm.querySelector('input[name="rate"]')?.value;
            if (rateId) {
                engineUrl = bookingEngineBaseUrl + encodeURIComponent(hotelId) + '/Rooms/Select?Rate=' + encodeURIComponent(rateId) + '&Arrival=' + encodeURIComponent(arrivalISO) + '&Departure=' + encodeURIComponent(departureISO);
            } else if (roomId) {
