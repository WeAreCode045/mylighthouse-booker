...
(function($) {
    ...
    function handleSpecialFallback($form, detail, bookingPageUrl) {
        try {
            ...
            var rateId = detail && detail.rate
                ? detail.rate
                : ($form.data('rate-id') || $form.data('special-id') || $form.find('input[name="special_id"]').val() || $form.find('input[name="rate_id"]').val() || '');
            ...
            if (target === 'modal' && typeof window.MLB_Modal !== 'undefined' && typeof window.MLB_Modal.openBookingModal === 'function') {
                window.MLB_Modal.openBookingModal(bookingPageUrl, hotelId, detail.arrivalISO, detail.departureISO, rateId, 'rate');
                return;
            }

            try {
                var redirectUrl = new URL(bookingPageUrl, window.location.origin);
                redirectUrl.searchParams.set('Arrival', detail.arrivalISO);
                redirectUrl.searchParams.set('Departure', detail.departureISO);
                if (hotelId) redirectUrl.searchParams.set('hotel_id', hotelId);
                if (rateId) redirectUrl.searchParams.set('rate', rateId);
            ...
            try {
                ...
                try {
                    if (modalSubmitBtn) {
                        ...
                        // Ensure disabled initially; it will be enabled on date select
                        modalSubmitBtn.disabled = true;
                        modalSubmitBtn.addEventListener('click', function() {
                            var arrivalDMY = $checkinHidden.val();
                            var departureDMY = $checkoutHidden.val();
                            if (!arrivalDMY || !departureDMY) {
                                alert( mlbGettext('Please select check-in and check-out dates.') );
                                return;
                            }
    ...
    function handleFormSubmission($form) {
        ...
        if (!checkin || !checkout) { alert( mlbGettext('Please select check-in and check-out dates.') ); return; }
        if (!$form.find('.mlb-daterange').val()) { alert( mlbGettext('Please select check-in and check-out dates.') ); return; }
        $form[0].submit();
    }
